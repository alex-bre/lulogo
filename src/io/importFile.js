 ...
 * those is what makes an exported picture round-trip.
 */
export async function readProjectFile(file) {
  if (isProjectFile(file)) return parseProject(await file.text())
  if (isSvgFile(file)) return extractProjectFromSvg(await file.text())
  if (isPngFile(file)) return extractProjectFromPng(new Uint8Array(await file.arrayBuffer()))
  return null
}
...
export async function openProjectFile(file) {
  ...
  if (!doc) {
    throw new Error(
      `“${file.name}” holds no project data. Open a .lulogo.json save, or an SVG/PNG ` +
        `exported with “Embed project source”.`,
    )
  }
  loadDocument(doc)
}
...
export async function importFiles(files, atWorld) {
  for (const file of files) {
    ...
    if (isSvgFile(file)) {
      ...
      if (await handledAsProject(extractProjectFromSvg(text), file, 'svg')) continue
      ...
      if (isPngFile(file)) {
        const project = extractProjectFromPng(new Uint8Array(await file.arrayBuffer()))
